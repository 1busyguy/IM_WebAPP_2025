...
import { PartnerAccountPage } from './PartnerAccountPage';
...
export const PartnerDetails: React.FC<PartnerDetailsProps> = ({
  ...
  const [showPartnerAccount, setShowPartnerAccount] = useState(false);

  // When showing partner account, render it as a full page
  if (showPartnerAccount) {
    return (
      <div className="fixed inset-0 bg-gray-100 z-50">
        <PartnerAccountPage
          partner={partner}
          onClose={() => setShowPartnerAccount(false)}
    ...
    <div className="h-full flex flex-col p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center space-x-2">
          <button
            onClick={() => setShowPartnerAccount(true)}
            className="p-2 hover:bg-gray-100 rounded-full transition-colors"
          >
